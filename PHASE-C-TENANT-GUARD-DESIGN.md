...
**⚠️ Establishment reliability (`run` vs `enterWith`).** `runInTenantScope` (`als.run(store, cb)`) is reliable: the scope is bound for the whole callback and reverts after — used to CONFINE work (login session write, portal module+contact validation) and for the fail-closed-on-null property. `enterTenantScope` (`als.enterWith`) sets the scope for "the rest of the request" WITHOUT a wrapping callback (the chokepoints use it after resolving the principal), but `enterWith` across `await` boundaries is a known Node foot-gun: it does not reliably override an already-set store or propagate back to a caller that already holds one. So the chokepoint `enterWith` is treated as **provisional** — correct for the common single-establishment case, dormant today — and the AUTHORITATIVE request→DB binding is finalised by the **RLS transaction bootstrap** (§2.2): each tenant request runs its DB work inside a `$transaction` that `set_config('app.tenant_id', …, true)`, which is the mechanism that actually confines queries at enforcement time. Tests therefore assert the resolution DECISION and the `run`-based confinement/no-leak properties, not `enterWith` cross-await propagation.
...
5. ✅ **`AppSetting.tenantId` additive slice** (decision 3) — DONE (migration `20260723120000`): nullable `tenantId` + index + backfill to `tenant_denago_cpt`, mirror of Phase B; the db.ts guard now scopes `AppSetting` transparently (extendedWhereUnique on the `key` findUnique + stamped upsert.create), so `getSetting`/`putSetting` read/write the request tenant with NO settings-layer change and install-global reads use a system-scope bypass — still inert while dormant. Cleared the last `PENDING` model in the schema-contract test. `key` stays the `@id` (one row per key); per-tenant values for the SAME key are deferred to the uniqueness batch (item 7). Follow-up: install-global writes that go through `basePrisma` (e.g. the backup cron's `BACKUP_LAST_RESULT`) land `tenantId = NULL` and are read via system bypass — fine dormant; revisit at the flip.
