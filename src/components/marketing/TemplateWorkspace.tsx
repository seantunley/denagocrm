...
function categoryFor(value: string) {
  return CATEGORIES.find((category) => category.value === value)
    ...
    ?? { value, label: value.replaceAll("_", " "), description: "Existing template category", channel: "email" as const };
}
...
export default function TemplateWorkspace({ templates }: { templates: Template[] }) {
  ...
  const [previewMode, setPreviewMode] = useState<"preview" | "plain">("preview");
  ...
  const canSave = draft.name.trim() && draft.body.trim() && (!isEmail || draft.subject.trim());
  ...
  function duplicate(template: Template) {
    setDraft({
      ...
      body: template.body,
      plainTextBody: template.plainTextBody ?? "",
      category: CATEGORIES.some((item) => item.value === template.category) ? template.category : "marketing_email",
    });
    setEditorOpen(true);
    setPreviewMode("preview");
    resetFeedback();
  ...
  function insertVariable(variable: string) {
    setDraft((current) => ({ ...current, body: `${current.body}${current.body && !current.body.endsWith(" ") ? " " : ""}${variable}` }));
  }
  ...
  async function publish(template: Template) {
    if (!window.confirm(`Publish “${template.name}”? Published templates are immutable; future changes should start from a new draft.`)) return;
    setPublishing(true);
    resetFeedback();
    try {
      await publishMarketingTemplate(template.id);
      setMessage(`“${template.name}” published.`);
      router.refresh();
    } catch (caught) {
      setError(caught instanceof Error ? caught.message : "Could not publish this template");
    } finally {
      setPublishing(false);
    }
  }
  ...
  async function archive(template: Template) {
    ...
    resetFeedback();
    try {
      await archiveMarketingTemplate(template.id);
      if (draft.id === template.id) closeEditor();
      router.refresh();
    } catch (caught) {
      setError(caught instanceof Error ? caught.message : "Could not archive this template");
    }
  }
        ...
        <div className="flex flex-col gap-4 border-b border-border p-5 sm:flex-row sm:items-center sm:justify-between sm:p-6">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-muted-foreground">Create once, preview clearly and publish an immutable version when it is ready.</p>
            </div>
          </div>
          {!editorOpen && (
          ...
          <div className="grid min-w-0 xl:grid-cols-[minmax(0,1fr)_26rem]">
            <div className="min-w-0 space-y-5 p-5 sm:p-6">
              <div className="flex items-start justify-between gap-3">
                <div>
                  <p className="text-xs font-medium uppercase tracking-[0.16em] text-primary">{draft.id ? "Edit draft" : "New template"}</p>
                  <h3 className="mt-1 text-lg font-semibold">{draft.name.trim() || "Compose reusable content"}</h3>
                </div>
                <button type="button" className="btn-secondary btn-sm" onClick={closeEditor}>
              ...
              <div className="grid gap-4 md:grid-cols-2">
                ...
                <div>
                  ...
                  <select
                    id="marketing-template-category"
                    className="input"
                    value={draft.category}
                    onChange={(event) => {
                      const category = event.target.value;
                      const next = categoryFor(category);
                      setDraft({ ...draft, category, subject: next.channel === "email" ? draft.subject : "" });
                    }}
                  >
                    {!CATEGORIES.some((item) => item.value === draft.category) && <option value={draft.category}>{categoryFor(draft.category).label}</option>}
              ...
              {isEmail && (
                <div>
                  <label className="label" htmlFor="marketing-template-subject">Email subject</label>
                  <input id="marketing-template-subject" className="input" value={draft.subject} onChange={(event) => setDraft({ ...draft, subject: event.target.value })} placeholder="Write a clear, useful subject line" />
                </div>
              )}
              ...
              <div>
                <div className="mb-2 flex flex-col gap-2 sm:flex-row sm:items-end sm:justify-between">
                  <div>
                    <label className="label mb-0" htmlFor="marketing-template-body">Message content</label>
                    <p className="mt-1 text-[11px] text-muted-foreground">{isEmail ? "HTML is supported, but you can write plain text too." : isSms ? "Keep it concise; the live preview shows the character count." : "Reusable content for internal notifications."}</p>
                  </div>
                  <div className="flex flex-wrap gap-1.5">
                    <button type="button" className="btn-secondary btn-sm" onClick={() => insertVariable("{{first_name}}")}>+ First name</button>
                    <button type="button" className="btn-secondary btn-sm" onClick={() => insertVariable("{{name}}")}>+ Full name</button>
                  </div>
                </div>
                <textarea
                  id="marketing-template-body"
                  className="input min-h-72 resize-y font-mono text-xs leading-6"
                  value={draft.body}
                  onChange={(event) => setDraft({ ...draft, body: event.target.value })}
                  placeholder={isSms ? "Hi {{first_name}}, your message…" : "Hi {{first_name}},\n\nWrite your reusable message here…"}
                />
              ...
              <div className="flex flex-wrap items-center gap-2 border-t border-border pt-5">
                <button type="button" className="btn-primary" onClick={save} disabled={saving || !canSave}>
                  <Save className="size-4" /> {saving ? "Saving…" : draft.id ? "Save new version" : "Save draft"}
                </button>
              ...
              <div className="sticky top-24 space-y-4">
                <div className="flex items-center justify-between gap-3">
                  <div>
                    ...
                    <p className="mt-1 text-xs text-muted-foreground">See what the reusable content looks like before saving.</p>
                  </div>
                  {isEmail && (
                    <div className="inline-flex rounded-lg border border-border bg-background/50 p-1 text-[11px]">
                      <button type="button" className={`rounded-md px-2 py-1 ${previewMode === "preview" ? "bg-primary text-primary-foreground" : "text-muted-foreground"}`} onClick={() => setPreviewMode("preview")}>Rendered</button>
                      <button type="button" className={`rounded-md px-2 py-1 ${previewMode === "plain" ? "bg-primary text-primary-foreground" : "text-muted-foreground"}`} onClick={() => setPreviewMode("plain")}>Plain</button>
                    </div>
                  )}
                </div>
                ...
                {isEmail ? (
                  <div className="overflow-hidden rounded-2xl border border-border bg-background">
                    <div className="border-b border-border p-3">
                      <div className="mb-1 flex items-center gap-2 text-[11px] text-muted-foreground"><Mail className="size-3.5" /> Email preview</div>
                      <p className="text-sm font-medium">{draft.subject.trim() || "Your email subject"}</p>
                    </div>
                    {previewMode === "plain" ? (
                      <pre className="min-h-72 whitespace-pre-wrap break-words p-4 font-sans text-xs leading-5">{draft.plainTextBody || stripMarkup(draft.body) || "Your message preview will appear here."}</pre>
                    ) : (
                      <iframe
                        title="Template email preview"
                        sandbox=""
                        srcDoc={draft.body || "<div style='font-family: sans-serif; color: #777; padding: 20px'>Your message preview will appear here.</div>"}
                  ...
                  <div className="rounded-2xl border border-border bg-background/60 p-4">
                    <div className="mb-4 flex items-center gap-2 text-[11px] text-muted-foreground"><MessageSquareText className="size-3.5" /> {isSms ? "SMS preview" : "Notification preview"}</div>
                    <div className="ml-auto max-w-[92%] rounded-2xl rounded-br-md bg-primary px-4 py-3 text-sm leading-5 text-primary-foreground">
                      {draft.body || "Your message preview will appear here."}
                    </div>
                  </div>
                )}
                ...
                <div className="rounded-xl border border-primary/20 bg-primary/5 p-3">
                  <div className="flex gap-2">
                    ...
                    <p className="text-[11px] leading-4 text-muted-foreground">Personalisation variables are resolved when the campaign or governed workflow sends the message. Published versions stay immutable for auditability.</p>
                  </div>
                </div>
              </div>
            </aside>
      ...
      <section className="space-y-3">
        <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
          <div>
            ...
            <p className="text-xs text-muted-foreground">Draft, publish and reuse templates without editing published content in place.</p>
          </div>
          <div className="flex flex-col gap-2 sm:flex-row">
            <div className="relative min-w-56">
              <Search className="pointer-events-none absolute left-3 top-1/2 size-4 -translate-y-1/2 text-muted-foreground" />
              <input className="input pl-9" value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search templates" />
            </div>
            <select className="input sm:w-auto" value={status} onChange={(event) => setStatus(event.target.value)}>
        ...
        {filtered.length === 0 ? (
          <div className="card border-dashed py-10 text-center">
            ...
            <p className="mt-3 font-medium">{templates.length === 0 ? "No templates yet" : "No templates match this filter"}</p>
            <p className="mt-1 text-xs text-muted-foreground">{templates.length === 0 ? "Create a reusable marketing or operational message." : "Try a different search or status."}</p>
            {templates.length === 0 && !editorOpen && <button type="button" className="btn-primary mt-4" onClick={openNew}><Plus className="size-4" /> Create template</button>}
          </div>
        ) : (
              ...
              return (
                <article key={template.id} className={`card flex min-h-60 flex-col ${template.status === "archived" ? "opacity-65" : ""}`}>
                  <div className="flex items-start gap-3">
                    <span className="grid size-9 shrink-0 place-items-center rounded-xl border border-primary/20 bg-primary/10 text-primary"><ChannelIcon className="size-4" /></span>
                    <div className="min-w-0 flex-1">
                      <div className="flex items-start justify-between gap-2">
                        <h3 className="truncate font-semibold">{template.name}</h3>
                        <span className="badge shrink-0">{template.status}</span>
                      </div>
                      <p className="mt-1 text-[11px] text-muted-foreground">{category.label} · v{template.version}</p>
                    </div>
                  </div>
                  ...
                  <div className="mt-4 flex flex-wrap gap-2 border-t border-border pt-3">
                    {template.status === "draft" && (
                      <button type="button" className="btn-secondary btn-sm" onClick={() => openEdit(template)}><Pencil className="size-3.5" /> Edit</button>
                    )}
                    ...
                    {template.status === "draft" && (
                      <button type="button" className="btn-primary btn-sm" disabled={publishing} onClick={() => publish(template)}><Send className="size-3.5" /> Publish</button>
                    )}
                    {template.status !== "archived" && (
                      <button type="button" className="btn-danger btn-sm" onClick={() => archive(template)}><Archive className="size-3.5" /> Archive</button>
                    )}
                  </div>
