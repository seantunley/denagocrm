  ...
  return (
    <div className={cn("space-y-6", className)}>
      <header className="relative overflow-hidden rounded-2xl border border-border bg-card p-4 shadow-sm sm:p-6">
        ...
        <div className="relative mt-4 flex flex-col gap-5 lg:flex-row lg:items-end lg:justify-between">
          <div className="min-w-0">
            ...
            <div className="flex min-w-0 flex-wrap items-center gap-2.5">
              <h1 className="min-w-0 text-2xl font-semibold tracking-[-0.04em] text-foreground sm:text-[30px]">{title}</h1>
              {status}
            </div>
          ...
          {actions && (
            <div className="flex shrink-0 flex-wrap items-center gap-2 border-t border-border pt-4 lg:max-w-[58%] lg:justify-end lg:border-0 lg:pt-0">
              {actions}
            </div>
          )}
        </div>
        ...
        {facts.length > 0 && (
          <dl className="relative mt-5 grid grid-cols-2 gap-px overflow-hidden rounded-xl border border-border bg-border sm:grid-cols-4">
            {facts.map((fact, index) => (
              <div key={index} className="min-w-0 bg-muted/35 px-3 py-2.5">
