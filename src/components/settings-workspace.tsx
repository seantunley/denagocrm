        ...
        <Surface className="absolute inset-x-0 top-[calc(100%+.5rem)] z-30 overflow-hidden bg-popover shadow-2xl">
          {matches.length ? (
            <ul className="divide-y divide-border">
              {matches.map((item) => (
                <li key={item.key}>
                  <Link href={settingsDestination(item)} onClick={() => setQuery("")} className="flex items-center gap-3 px-4 py-3 transition hover:bg-accent/60">
        ...
        return (
          <Surface key={group.label} className="p-5">
            <div className="flex items-center gap-3">
              <span className="grid size-10 place-items-center rounded-xl border border-primary/20 bg-primary/10 text-primary"><Icon className="size-[18px]" /></span>
            ...
            <div className="mt-4 divide-y divide-border border-t border-border">
              {group.items.map((item) => (
                <Link key={item.key} href={settingsDestination(item)} className="flex items-center justify-between gap-3 py-3 text-sm text-muted-foreground transition hover:text-foreground">
...
export function SettingsSection({
  ...
  return (
    <section className={cn("overflow-hidden rounded-2xl border border-border bg-card shadow-sm", className)}>
      <header className="flex flex-col gap-3 border-b border-border bg-muted/20 p-4 sm:flex-row sm:items-start sm:justify-between sm:p-5">
        <div className="flex min-w-0 items-start gap-3">
          <span className="grid size-9 shrink-0 place-items-center rounded-xl border border-primary/20 bg-primary/10 text-primary"><Icon className="size-[18px]" /></span>
...
export function SettingsIntegrationRow({
  ...
  return (
    <details className="group border-b border-border last:border-b-0">
      <summary className="flex cursor-pointer list-none items-center justify-between gap-4 px-4 py-4 transition hover:bg-muted/20 [&::-webkit-details-marker]:hidden sm:px-5">
      ...
      <div className="border-t border-border bg-background/20 px-4 py-5 sm:px-5">{children}</div>
    </details>
  );
}
