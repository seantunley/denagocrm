  ...
  return (
    ...
    >
      ...
      <div className="flex items-start gap-2.5">
        <GripVertical className="mt-0.5 size-4 shrink-0 text-muted-foreground/35 transition-colors group-hover:text-muted-foreground" aria-hidden="true" />
            ...
            <div className="mt-1 flex min-w-0 flex-wrap items-center gap-1.5">
              {lead.productName && (
                <span className="max-w-full truncate rounded-md border border-white/[0.06] bg-white/[0.035] px-1.5 py-0.5 text-[10px] font-medium text-muted-foreground">
                  {lead.productName}{lead.color ? ` · ${lead.color}` : ""}
                </span>
              )}
      ...
      <div className="mt-3 rounded-lg border border-border/70 bg-background/30 px-2.5 py-2">
        {lead.nextStep ? (
          <div className="flex items-start gap-2">
            <CalendarClock className={cn("mt-0.5 size-3.5 shrink-0", lead.nextStep.overdue ? "text-red-300" : "text-primary")} />
            <div className="min-w-0 flex-1">
              <p className="truncate text-[11px] font-medium text-foreground">{lead.nextStep.summary}</p>
              <p className={cn("mt-0.5 text-[10px]", lead.nextStep.overdue ? "font-medium text-red-300" : "text-muted-foreground")}>{lead.nextStep.when}</p>
            </div>
          </div>
        ) : (
          <div className="flex items-center gap-2 text-[10px] text-amber-300">
            <CircleAlert className="size-3.5" />No next step scheduled
          </div>
        )}
      </div>
            ...
            <Tooltip>
              <TooltipTrigger asChild>
                <span className="flex items-center gap-1 rounded-md bg-amber-500/10 px-1.5 py-0.5 text-[10px] font-medium text-amber-300">
                  <Hourglass className="size-3" />{lead.ageDays} days in stage
                </span>
              </TooltipTrigger>
      ...
      <header className="relative border-b border-border bg-card/95 px-4 py-3.5 backdrop-blur">
        ...
        <div className="flex items-center justify-between gap-3">
          <div className="flex min-w-0 items-center gap-2">
            <span className="size-2.5 shrink-0 rounded-full shadow-[0_0_12px_currentColor]" style={{ backgroundColor: stage.color, color: stage.color }} />
            <h3 id={`pipeline-stage-title-${stage.id}`} className="truncate text-sm font-semibold text-foreground">{stage.name}</h3>
        ...
        <p className="mt-1.5 text-[10px] text-muted-foreground">{stage.leads.length === 1 ? "1 active opportunity" : `${stage.leads.length} active opportunities`}</p>
      </header>
      <div
        ref={setNodeRef}
        className={cn(
      ...
      >
        ...
        {stage.leads.length === 0 && !isOver && (
          <div className="flex min-h-32 flex-col items-center justify-center rounded-xl border border-dashed border-border/70 bg-card/25 px-5 text-center">
            <span className="grid size-8 place-items-center rounded-full border border-border bg-muted/40 text-muted-foreground"><GripVertical className="size-3.5" /></span>
            <p className="mt-3 text-xs font-medium text-muted-foreground">Drop a lead into {stage.name}</p>
            <p className="mt-1 text-[10px] leading-4 text-muted-foreground/65">Stage rules and automations run after the move.</p>
          </div>
        )}
      </div>
    </section>
...
export default function KanbanBoard({
  ...
  const owners = useMemo(
    () => Array.from(new Set(stages.flatMap((stage) => stage.leads.map((lead) => lead.assignee).filter(Boolean) as string[]))).sort(),
    [stages],
  );
  const visibleStages = useMemo(() => {
    ...
    return stages.map((stage) => ({
      ...
      leads: stage.leads.filter((lead) => {
        const matchesQuery = !needle || [lead.name, lead.title, lead.productName, lead.color, lead.source]
          .filter(Boolean)
          .join(" ")
          .toLowerCase()
          .includes(needle);
        const matchesOwner = !owner || lead.assignee === owner;
        const needsAttention = !attentionOnly || lead.noNextStep || (lead.ageDays ?? 0) >= STALE_DAYS;
  ...
  return (
      ...
      <div className="rounded-2xl border border-border bg-card/70 p-3 shadow-sm backdrop-blur sm:p-4">
        <div className="flex flex-col gap-3 xl:flex-row xl:items-center xl:justify-between">
          <div className="flex min-w-0 flex-1 flex-col gap-2 sm:flex-row">
            ...
            <select value={owner} onChange={(event) => setOwner(event.target.value)} className="input h-10 sm:w-44" aria-label="Filter by owner">
            ...
            <button
              ...
              className={cn("btn-secondary h-10", attentionOnly && "border-amber-400/35 bg-amber-500/10 text-amber-200")}
              aria-pressed={attentionOnly}
            >
              <CircleAlert className="size-4" />Needs attention
            </button>
            {(query || owner || attentionOnly) && (
              <button type="button" onClick={() => { setQuery(""); setOwner(""); setAttentionOnly(false); }} className="btn h-10 text-muted-foreground">
                <FilterX className="size-4" />Clear
              </button>
            )}
          </div>
          <div className="flex items-center justify-between gap-3">
            <p className="text-xs text-muted-foreground"><span className="font-semibold tabular-nums text-foreground">{visibleCount}</span>{visibleCount !== totalCount ? ` of ${totalCount}` : ""} opportunities</p>
            <div className="hidden items-center gap-1 md:flex">
              <Button type="button" variant="outline" size="icon-sm" onClick={() => scrollBoard(-1)} aria-label="Previous pipeline stages"><ChevronLeft className="size-4" /></Button>
              <Button type="button" variant="outline" size="icon-sm" onClick={() => scrollBoard(1)} aria-label="Next pipeline stages"><ChevronRight className="size-4" /></Button>
            </div>
          </div>
        </div>

        <nav className="mt-3 flex gap-1.5 overflow-x-auto border-t border-border pt-3 md:hidden" aria-label="Pipeline stages">
          {visibleStages.map((stage) => (
            <button key={stage.id} type="button" onClick={() => scrollToStage(stage.id)} className="inline-flex shrink-0 items-center gap-1.5 rounded-full border border-border bg-muted/35 px-3 py-1.5 text-xs font-medium text-muted-foreground">
              <span className="size-1.5 rounded-full" style={{ backgroundColor: stage.color }} />{stage.name}<span className="tabular-nums">{stage.leads.length}</span>
            </button>
          ))}
        </nav>
      </div>
    ...
    >
      <div ref={boardRef} className="flex snap-x snap-mandatory select-none gap-4 overflow-x-auto pb-3 pt-1 [scrollbar-gutter:stable]">
        {visibleStages.map((stage) => (
