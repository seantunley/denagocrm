      ...
      <div className="relative p-4 sm:px-5 sm:py-4">
        <div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
          <div className="flex min-w-0 items-start gap-3 sm:items-center">
            <span className="grid size-10 shrink-0 place-items-center rounded-xl border border-primary/20 bg-primary/10 text-primary shadow-sm sm:size-9">
            ...
            <div className="min-w-0 xl:flex xl:items-center xl:gap-4">
              <div className="shrink-0">
                ...
                <h1 className="mt-1 text-xl font-semibold tracking-[-0.04em] text-foreground sm:mt-0.5">{title}</h1>
              </div>
              <p className="mt-1.5 max-w-2xl text-xs leading-5 text-muted-foreground xl:mt-0 xl:max-w-xl">{description}</p>
            </div>
          </div>
        ...
        {stats.length > 0 ? (
          <div className={cn(
            "mt-3 grid grid-cols-2 gap-px overflow-hidden rounded-lg border border-border/70 bg-border/70",
            ...
            {stats.map((stat, index) => {
              ...
              return <div key={index} className="min-w-0 bg-background/55 px-3 py-2 backdrop-blur-sm">
                ...
                <p className={cn("mt-0.5 text-base font-semibold tracking-[-0.03em] tabular-nums", STAT_TONES[stat.tone ?? "default"])}>{stat.value}</p>
