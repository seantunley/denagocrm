...
function NodePanel({ node, isStart, nodeOptions, variables, onChange, onDelete, onMakeStart }: { node: FlowNode; isStart: boolean; nodeOptions: { id: string; label: string }[]; variables: string[]; onChange: (n: FlowNode) => void; onDelete: () => void; onMakeStart: () => void }) {
  ...
  return (
    <div className="space-y-3">
      ...
      {(node.type === "message" || node.type === "answer" || node.type === "capture" || node.type === "captureFile" || node.type === "image" || node.type === "booking" || node.type === "slots") && <div><label className="label">Then go to</label><TargetPicker nodeOptions={nodeOptions} value={(node as { next?: string }).next} onPick={(v) => onChange({ ...node, next: v } as FlowNode)} /></div>}
