...
import {
  Bot, CalendarDays, CircleStop, FileQuestion, FileUp, GitBranch, Hand, ImageIcon,
  Maximize2, MessageSquare, Minimize2, PanelLeftClose, PanelLeftOpen,
  PanelRightClose, PanelRightOpen, RotateCcw, Save, Sparkles, Upload, Workflow,
...
function summary(n: FlowNode): string {
  ...
  if (n.type === "condition") return `{{${n.condition.variable || "variable"}}} ${n.condition.operator.replace("_", " ")}${n.condition.value ? ` “${n.condition.value}”` : ""}`;
  if (n.type === "ai") return "Chats, grounded in approved facts";
  if (n.type === "choice") return n.text.slice(0, 50);
  return "";
}

function NodeCard({ data }: NodeProps) {
  const d = data as unknown as RFData; const n = d.flow; const meta = TYPE_META[n.type]; const Icon = meta.icon;
  return (
    <div className={cn("w-56 rounded-xl border bg-slate-950/95 text-slate-100 shadow-xl", d.isStart ? "border-orange-400/70 ring-2 ring-orange-400/15" : meta.tone)}>
      ...
      <div className={cn("flex items-center gap-1.5 rounded-t-[11px] px-3 py-2 text-xs font-semibold", meta.header)}><Icon className="size-3.5" />{meta.label}{d.isStart && <span className="ml-auto text-[10px] bg-orange-600 px-1.5 rounded">START</span>}</div>
      <div className="px-3 py-2 text-xs text-slate-300 min-h-8 whitespace-pre-wrap">{summary(n) || "…"}</div>
      {n.type === "choice" ? <div className="pb-1">{n.options.map((o, i) => <div key={o.id} className="relative px-3 py-1 text-[11px] text-slate-400 border-t border-slate-800">{o.label}<Handle type="source" position={Position.Right} id={`opt:${o.id}`} style={{ top: `${100 + i * 26}px`, background: meta.handle }} /></div>)}</div>
      : n.type === "condition" ? <div className="pb-1"><div className="relative border-t border-slate-800 px-3 py-1 text-[11px] text-emerald-300">Yes<Handle type="source" position={Position.Right} id="true" style={{ top: "92px", background: "#34d399" }} /></div><div className="relative border-t border-slate-800 px-3 py-1 text-[11px] text-red-300">No<Handle type="source" position={Position.Right} id="false" style={{ top: "118px", background: "#f87171" }} /></div></div>
      : n.type === "ai" ? <Handle type="source" position={Position.Right} id="handoff" style={{ background: meta.handle }} />
      : n.type === "handoff" || n.type === "end" ? null : <Handle type="source" position={Position.Right} id="out" style={{ background: meta.handle }} />}
    </div>
  );
}
const nodeTypes = { flowNode: NodeCard };
let seq = 0; const newId = (type: string) => `${type}_${Date.now().toString(36)}${seq++}`;
...
export default function FlowBuilder({ flowId, initial, journeys = [] }: { flowId: string; initial: FlowData; journeys?: FlowJourneyOption[] }) {
  const router = useRouter(); const [start, setStart] = useState(initial.start);
  const [rfNodes, setRfNodes, onNodesChange] = useNodesState<Node<RFData>>(Object.values(initial.nodes).map((n, i) => ({ id: n.id, type: "flowNode", position: initial.positions?.[n.id] ?? { x: (i % 4) * 300 + 40, y: Math.floor(i / 4) * 220 + 40 }, data: { flow: n, isStart: n.id === initial.start } })));
  const [selectedId, setSelectedId] = useState<string | null>(null); const [status, setStatus] = useState("Saved"); const [fullscreen, setFullscreen] = useState(false); const [paletteOpen, setPaletteOpen] = useState(false); const [inspectorOpen, setInspectorOpen] = useState(false);
  const patch = useCallback((id: string, updater: (n: FlowNode) => FlowNode) => { setStatus("Unsaved changes"); setRfNodes((ns) => ns.map((rn) => rn.id === id ? { ...rn, data: { ...rn.data, flow: updater(rn.data.flow) } } : rn)); }, [setRfNodes]);

  const edges: Edge[] = useMemo(() => {
    const out: Edge[] = [];
    for (const rn of rfNodes) {
      const n = rn.data.flow; const add = (handle: string, target?: string) => { if (target && rfNodes.some((x) => x.id === target)) out.push({ id: `${n.id}:${handle}`, source: n.id, sourceHandle: handle, target, animated: true, style: { stroke: "#475569" } }); };
      if (n.type === "choice") n.options.forEach((o) => add(`opt:${o.id}`, o.next)); else if (n.type === "condition") { add("true", n.trueNext); add("false", n.falseNext); } else if (n.type === "ai") add("handoff", n.handoffNext); else if (n.type !== "handoff" && n.type !== "end") add("out", (n as { next?: string }).next);
    }
    return out;
  }, [rfNodes]);
  ...
  const onConnect = useCallback((c: Connection) => {
    ...
    patch(c.source, (n) => {
      if (n.type === "choice" && c.sourceHandle!.startsWith("opt:")) { const oid = c.sourceHandle!.slice(4); return { ...n, options: n.options.map((o) => o.id === oid ? { ...o, next: c.target! } : o) }; }
      if (n.type === "condition") { if (c.sourceHandle === "true") return { ...n, trueNext: c.target! }; if (c.sourceHandle === "false") return { ...n, falseNext: c.target! }; }
      if (n.type === "ai" && c.sourceHandle === "handoff") return { ...n, handoffNext: c.target! };
      return { ...n, next: c.target! } as FlowNode;
    });
  }, [patch]);

  function addNode(type: FlowNode["type"]) { const id = newId(type); setRfNodes((ns) => [...ns, { id, type: "flowNode", position: { x: 120 + Math.random() * 200, y: 120 + Math.random() * 200 }, data: { flow: blankNode(type, id), isStart: false } }]); setSelectedId(id); setStatus("Unsaved changes"); setInspectorOpen(true); }
  function removeNode(id: string) { setRfNodes((ns) => ns.filter((n) => n.id !== id).map((n) => ({ ...n, data: { ...n.data, flow: clearRefs(n.data.flow, id) } }))); if (selectedId === id) setSelectedId(null); setStatus("Unsaved changes"); }
  function markStart(id: string) { setStart(id); setRfNodes((ns) => ns.map((n) => ({ ...n, data: { ...n.data, isStart: n.id === id } }))); setStatus("Unsaved changes"); }
  async function onSave() {
    setStatus("Saving…"); const nodes: Record<string, FlowNode> = {}; const positions: Record<string, Pos> = {};
    for (const rn of rfNodes) { nodes[rn.id] = rn.data.flow; positions[rn.id] = rn.position; }
    const res = await saveFlow(flowId, JSON.stringify({ start, nodes, positions })); setStatus(res.ok ? "Saved" : res.error ?? "Save failed");
    if (res.ok) { toast.success("Flow saved"); router.refresh(); } else toast.error(res.error ?? "Flow could not be saved");
  }
  ...
  const knownVariables = useMemo(() => { const vars = new Set(BUILTIN_VARIABLES); for (const rn of rfNodes) { const node = rn.data.flow; if (node.type === "capture" || node.type === "captureFile") if (node.variable) vars.add(node.variable); } return [...vars].sort(); }, [rfNodes]);
  ...
  return (
    <BuilderWorkspaceShell fullscreen={fullscreen} className="min-h-[720px] md:h-[calc(100dvh-8rem)] md:min-h-0">
      <BuilderWorkspaceBar title="Conversation flow" description="Connect nodes to shape customer conversations and CRM hand-offs." status={<BuilderSaveStatus status={status} />}>
        <div className="flex items-center rounded-lg border border-white/10 bg-white/[0.035] p-0.5"><button type="button" onClick={() => setPaletteOpen((v) => !v)} className={cn("flex h-8 items-center gap-1.5 rounded-md px-2.5 text-xs text-slate-300 hover:bg-white/7", paletteOpen && "bg-primary/15 text-primary")}>{paletteOpen ? <PanelLeftClose className="size-4" /> : <PanelLeftOpen className="size-4" />} Nodes</button><button type="button" onClick={() => setInspectorOpen((v) => !v)} className={cn("flex h-8 items-center gap-1.5 rounded-md px-2.5 text-xs text-slate-300 hover:bg-white/7", inspectorOpen && "bg-primary/15 text-primary")}>{inspectorOpen ? <PanelRightClose className="size-4" /> : <PanelRightOpen className="size-4" />} Inspector</button></div>
        <button type="button" onClick={onSave} className="btn-primary btn-sm"><Save className="size-4" />Save</button>
        <ConfirmActionDialog destructive title="Reset this flow?" description="Every node, connection and unsaved change will be replaced with the default flow." confirmLabel="Reset flow" onConfirm={async () => { await resetFlow(flowId); toast.success("Flow reset"); router.refresh(); }} trigger={<button type="button" className="btn-secondary btn-sm"><RotateCcw className="size-4" />Reset</button>} />
        <button type="button" onClick={() => setFullscreen((v) => !v)} className="btn-secondary btn-sm">{fullscreen ? <Minimize2 className="size-4" /> : <Maximize2 className="size-4" />}<span className="hidden sm:inline">{fullscreen ? "Exit" : "Full screen"}</span></button>
      </BuilderWorkspaceBar>

      <div className="relative flex min-h-0 flex-1">
        {paletteOpen && <aside className="w-60 shrink-0 overflow-y-auto border-r border-white/[0.08] bg-[#111614] max-md:fixed max-md:inset-x-0 max-md:bottom-0 max-md:z-[80] max-md:max-h-[72dvh] max-md:w-auto max-md:rounded-t-3xl max-md:border-t"><div className="sticky top-0 z-10 flex items-center justify-between border-b border-white/[0.08] bg-[#111614]/95 px-4 py-3"><div><p className="text-sm font-semibold text-white">Node palette</p><p className="text-xs text-slate-400">Add a step to the canvas</p></div><button type="button" onClick={() => setPaletteOpen(false)} className="grid size-8 place-items-center rounded-lg text-slate-400"><X className="size-4" /></button></div><div className="grid gap-2 p-3">{(Object.keys(TYPE_META) as FlowNode["type"][]).map((type) => { const meta = TYPE_META[type]; const Icon = meta.icon; return <button key={type} type="button" onClick={() => addNode(type)} className={cn("flex items-center gap-3 rounded-xl border bg-white/[0.025] px-3 py-2.5 text-left text-sm text-slate-200", meta.tone)}><span className={cn("grid size-8 place-items-center rounded-lg", meta.header)}><Icon className="size-4" /></span><span>{meta.label}</span></button>; })}</div></aside>}
        <main className="min-w-0 flex-1 bg-[#0b0f0e] p-2 sm:p-3"><div className="h-full min-h-[32rem] overflow-hidden rounded-xl border border-white/[0.08] bg-[#0f1412]"><ReactFlow nodes={rfNodes} edges={edges} nodeTypes={nodeTypes} onNodesChange={onNodesChange} onConnect={onConnect} onNodeClick={(_, node) => { setSelectedId(node.id); setInspectorOpen(true); }} onPaneClick={() => setSelectedId(null)} fitView proOptions={{ hideAttribution: true }}><Background color="#25312d" gap={20} /><Controls className="!border-white/10 !bg-[#18201d] !text-white" /></ReactFlow></div></main>
        {inspectorOpen && <aside className="w-80 shrink-0 overflow-y-auto border-l border-white/[0.08] bg-[#111614] max-md:fixed max-md:inset-x-0 max-md:bottom-0 max-md:z-[81] max-md:max-h-[78dvh] max-md:w-auto max-md:rounded-t-3xl max-md:border-t"><div className="sticky top-0 z-10 flex items-center justify-between border-b border-white/[0.08] bg-[#111614]/95 px-4 py-3"><div><p className="text-sm font-semibold text-white">Inspector</p><p className="text-xs text-slate-400">Configure the selected node</p></div><button type="button" onClick={() => setInspectorOpen(false)} className="grid size-8 place-items-center rounded-lg text-slate-400"><X className="size-4" /></button></div><div className="p-4 space-y-5">{!selected ? <p className="text-sm leading-6 text-slate-400">Select a node to edit it.</p> : <NodePanel key={selected.id} node={selected} isStart={selected.id === start} nodeOptions={nodeOptions.filter((o) => o.id !== selected.id)} variables={knownVariables} journeys={journeys} onChange={(next) => patch(selected.id, () => next)} onDelete={() => removeNode(selected.id)} onMakeStart={() => markStart(selected.id)} />}<VariablesPanel variables={knownVariables} /></div></aside>}
      </div>
...
function clearRefs(n: FlowNode, removedId: string): FlowNode {
  ...
  const nn = n as { next?: string }; return nn.next === removedId ? ({ ...n, next: undefined } as FlowNode) : n;
}
function TargetPicker({ value, onPick, nodeOptions }: { value?: string; onPick: (value?: string) => void; nodeOptions: { id: string; label: string }[] }) { return <select className="input btn-sm" value={value ?? ""} onChange={(e) => onPick(e.target.value || undefined)}><option value="">— (ends here) —</option>{nodeOptions.map((o) => <option key={o.id} value={o.id}>{o.label}</option>)}</select>; }
function VariablesPanel({ variables }: { variables: string[] }) { return <div className="border-t border-white/8 pt-4"><p className="text-xs font-semibold uppercase tracking-wide text-slate-400">Variables</p><p className="mt-1 text-xs text-slate-500">Use these in messages as <code>{"{{variable}}"}</code> or in a Condition node.</p><div className="mt-2 flex flex-wrap gap-1.5">{variables.map((v) => <code key={v} className="rounded-md border border-white/8 bg-white/[0.035] px-1.5 py-1 text-[11px] text-slate-300">{`{{${v}}}`}</code>)}</div></div>; }
...
function NodePanel({ node, isStart, nodeOptions, variables, journeys, onChange, onDelete, onMakeStart }: { node: FlowNode; isStart: boolean; nodeOptions: { id: string; label: string }[]; variables: string[]; journeys: FlowJourneyOption[]; onChange: (n: FlowNode) => void; onDelete: () => void; onMakeStart: () => void }) {
  const meta = TYPE_META[node.type]; const Icon = meta.icon;
