...
export async function POST(req: NextRequest) {
  // Authenticate the API key before any guarded read, then keep its resolved
  // workspace as an ENCLOSING scope for the whole verification operation. A
  // helper-level enterWith followed by a return to this Route Handler is not a
  // reliable carrier under enforcement.
  {
  ...
  return withTenantScopeFromId(auth.tenantId, async () => {
    ...
    // [a-zA-Z0-9] above, so the wildcards can't reach the query today; the
