...
export async function previewMarketingAudience(formData: FormData) {
  ...
  const tree = json<AudienceGroup>(formData.get("ruleTree"));
  const contacts = await evaluateAudience(tree, String(formData.get("channel") ?? "any"));
...
export async function archiveMarketingAudience(id: string) {
  const user = await requirePermission("campaigns.manage_audiences");
  const tenantId = await getActiveTenantId();
  const used = await basePrisma.$queryRaw<Array<{ count: bigint }>>`SELECT COUNT(*)::bigint AS count FROM "Campaign" WHERE "tenantId" IS NOT DISTINCT FROM ${tenantId} AND "segmentId" = ${id}`;
  if (Number(used[0]?.count ?? 0) > 0) {
    await basePrisma.$executeRaw`UPDATE "Segment" SET "status" = 'archived', "archivedAt" = CURRENT_TIMESTAMP WHERE "id" = ${id} AND "tenantId" IS NOT DISTINCT FROM ${tenantId}`;
  } else {
    await basePrisma.$executeRaw`UPDATE "Segment" SET "status" = 'archived', "archivedAt" = CURRENT_TIMESTAMP WHERE "id" = ${id} AND "tenantId" IS NOT DISTINCT FROM ${tenantId}`;
  }
...
export async function saveMarketingTemplate(formData: FormData) {
  ...
  `;
  const rows = await basePrisma.$queryRaw<Array<{ version: number }>>`SELECT COALESCE(MAX("version"), 0) + 1 AS version FROM "MarketingTemplateVersion" WHERE "templateId" = ${id}`;
  ...
  await basePrisma.$executeRaw`UPDATE "EmailTemplate" SET "version" = ${version} WHERE "id" = ${id}`;
...
export async function publishMarketingTemplate(id: string) {
  const user = await requirePermission("campaigns.manage_templates");
  const tenantId = await getActiveTenantId();
  await basePrisma.$executeRaw`UPDATE "EmailTemplate" SET "status" = 'published', "updatedAt" = CURRENT_TIMESTAMP WHERE "id" = ${id} AND "tenantId" IS NOT DISTINCT FROM ${tenantId}`;
...
export async function archiveMarketingTemplate(id: string) {
  const user = await requirePermission("campaigns.manage_templates");
  const tenantId = await getActiveTenantId();
  await basePrisma.$executeRaw`UPDATE "EmailTemplate" SET "status" = 'archived', "archivedAt" = CURRENT_TIMESTAMP WHERE "id" = ${id} AND "tenantId" IS NOT DISTINCT FROM ${tenantId}`;
