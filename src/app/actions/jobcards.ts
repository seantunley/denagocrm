  ...
  const jobCard = await basePrisma.$transaction(async (tx) => {
    ...
    if (jc.kmIn != null) {
      await tx.mileageLog.create({
        data: { vehicleId, km: jc.kmIn, note: `Job card #${jc.number} check-in`, tenantId: await actingTenantId() },
      });
    }
    return jc;
  });
...
export async function addJobCardItem(jobCardId: string, formData: FormData) {
  return asActionResult(async () => {
    ...
    const outcome = await basePrisma.$transaction(async (tx) => {
      ...
      await tx.jobCardItem.create({ data: { tenantId: await actingTenantId(), jobCardId, kind, description, qty, unitPriceCents, partId } });
  ...
  return asActionResult(async () => {
    ...
    const outcome = await basePrisma.$transaction(async (tx) => {
      for (const item of pkg.items) {
        ...
        await tx.jobCardItem.create({
          data: { jobCardId, kind: item.kind, description: item.description, qty: item.qty, unitPriceCents: item.unitPriceCents, partId: item.partId, tenantId: await actingTenantId() },
        });
      }
      return { ok: true as const };
    });
