  ...
  return asActionResult(async () => {
    ...
    if (encoded) {
      try {
        ...
        throw new ActionRefusal("Purchase-order lines are invalid");
      }
    }
    if (!lines.length) {
      const productId = str(formData.get("productId"));
      if (!productId) refuse("Choose a product for the purchase order.");
      lines = [{
        productId,
  ...
  return asActionResult(async () => {
    ...
    if (linesRaw) {
      try {
        ...
        throw new ActionRefusal("Receiving quantities are invalid");
      }
    }
    ...
    const received = await prisma.$transaction(async (tx) => {
      ...
      if (!po || !["ordered", "partially_received"].includes(po.status)) {
        throw new ActionRefusal("This purchase order cannot be received");
      }
      ...
      if (!incoming.length) throw new ActionRefusal("There are no outstanding units on this order");
      ...
      if (!chosen.length) throw new ActionRefusal("Choose at least one unit to receive");
      ...
      if (!claimed.length) throw new ActionRefusal("Those units were just received by someone else — refresh and try again.");
...
export async function cancelPurchaseOrder(id: string, formData?: FormData) {
  return asActionResult(async () => {
    ...
    const count = await prisma.$transaction(async (tx) => {
      ...
      await tx.$executeRaw`SELECT id FROM "PurchaseOrder" WHERE id = ${id} FOR UPDATE`;
      const po = await tx.purchaseOrder.findFirst({ where: { id, deletedAt: null } });
      if (!po || !["ordered", "partially_received"].includes(po.status)) throw new ActionRefusal("Only open purchase orders can be cancelled");
...
export async function addStockUnit(formData: FormData) {
  return asActionResult(async () => {
    ...
    refresh(unit.id);
    return { redirectTo: `/stock/${unit.id}` };
  });
}
...
export async function inspectReceivedUnit(id: string, formData: FormData) {
  return asActionResult(async () => {
    const user = await requirePermission("stock.manage");
    const current = await activeUnit(id);
    if (current.status !== "received_pending_check") throw new ActionRefusal("Only newly received units can be inspected");
...
export async function reserveUnit(id: string, formData: FormData) {
  return asActionResult(async () => {
    // Authorise before refusing: an unauthorised caller must not learn anything
    // from the shape of the refusal.
    await requirePermission("stock.manage");
    const leadId = str(formData.get("leadId"));
    if (!leadId) refuse("Choose a lead");
    const user = await requireLeadAccess(leadId, "stock.manage");
    const current = await activeUnit(id);
    if (current.status !== "available") throw new ActionRefusal("Only available stock can be reserved");
    const lead = await prisma.lead.findUnique({ where: { id: leadId }, select: { name: true, productId: true, status: true } });
    if (!lead || lead.status !== "open") throw new ActionRefusal("Only an open lead can reserve stock");
    if (lead.productId && lead.productId !== current.productId) throw new ActionRefusal("This unit does not match the lead's selected product");
    ...
    if (!changed) throw new ActionRefusal("This unit was reserved by someone else. Refresh and choose another unit.");
...
export async function releaseUnit(id: string, formData?: FormData) {
  return asActionResult(async () => {
    const user = await requirePermission("stock.manage");
    const current = await activeUnit(id);
    if (!["reserved", "allocated"].includes(current.status)) throw new ActionRefusal("Only reserved or allocated stock can be released");
...
export async function allocateUnit(id: string, formData: FormData) {
  return asActionResult(async () => {
    ...
    if (!quoteId) refuse("Choose an accepted quote");
    const user = await requireQuoteAccess(quoteId, "stock.manage");
    const current = await activeUnit(id);
    if (!["available", "reserved"].includes(current.status)) throw new ActionRefusal("This unit cannot be allocated from its current status");
    const quote = await prisma.quote.findUnique({ where: { id: quoteId }, include: { items: true, lead: true, contact: true } });
    if (!quote || quote.status !== "accepted") throw new ActionRefusal("Stock can only be allocated to an accepted quote");
    const matches = quote.items.some((item) => item.productId === current.productId && item.selected);
    if (!matches) throw new ActionRefusal("The accepted quote does not contain this product");
    ...
    if (!changed) throw new ActionRefusal("The stock unit changed while you were allocating it");
...
export async function transitionStockUnit(id: string, formData: FormData) {
  return asActionResult(async () => {
    ...
    if (!canTransitionStock(current.status, toStatus)) throw new ActionRefusal(`Cannot move stock from ${current.status} to ${toStatus}`);
    ...
    if (result.count !== 1) throw new ActionRefusal("The stock unit changed while this action was being processed");
...
export async function completePdi(id: string, formData: FormData) {
  return asActionResult(async () => {
    const user = await requirePermission("stock.manage");
    const current = await activeUnit(id);
    if (current.status !== "pdi") throw new ActionRefusal("The unit must be in PDI before it can be completed");
    ...
    if (result.count !== 1) throw new ActionRefusal("The unit changed while PDI was being completed");
...
export async function setStockUnitLabel(id: string, formData: FormData) {
  return asActionResult(async () => {
    ...
    if (slug) {
      const labels = await getStockLabels();
      if (!labels.some((l) => l.slug === slug)) throw new ActionRefusal("Unknown stock label");
    }
...
export async function deleteStockUnit(id: string, formData?: FormData) {
  return asActionResult(async () => {
    ...
    if (["reserved", "allocated", "pdi", "ready_for_delivery", "delivered", "sold"].includes(current.status)) {
      throw new ActionRefusal("Release or complete this unit's active workflow before removing it");
    }
