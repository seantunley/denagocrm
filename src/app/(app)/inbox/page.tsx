...
export default async function InboxPage() {
  ...
  return (
    <div className="space-y-6">
      ...
      <Tabs
        tabs={[
          { key: "all", label: "All", count: unread, content: <SocialThreadList collaboration={collaboration} staff={collabStaff} canCollaborate={canCollaborate} list={threadList} empty="No conversations yet. Messages appear here as soon as a connected customer channel receives one." /> },
          { key: "whatsapp", label: "WhatsApp", count: threadList.filter((thread) => thread.channel === "whatsapp" && thread.unread).length, content: <SocialThreadList collaboration={collaboration} staff={collabStaff} canCollaborate={canCollaborate} list={threadList.filter((thread) => thread.channel === "whatsapp")} empty="No WhatsApp conversations yet. Connect the WhatsApp Business number in Settings → Integrations." /> },
          { key: "messenger", label: "Messenger", count: threadList.filter((thread) => thread.channel === "messenger" && thread.unread).length, content: <SocialThreadList collaboration={collaboration} staff={collabStaff} canCollaborate={canCollaborate} list={threadList.filter((thread) => thread.channel === "messenger")} empty="No Messenger conversations yet." /> },
          { key: "instagram", label: "Instagram", count: threadList.filter((thread) => thread.channel === "instagram" && thread.unread).length, content: <SocialThreadList collaboration={collaboration} staff={collabStaff} canCollaborate={canCollaborate} list={threadList.filter((thread) => thread.channel === "instagram")} empty="No Instagram DMs yet. They appear once the Instagram account and Meta messaging permissions are connected." /> },
          { key: "reviews", label: "Google Reviews", count: reviews.length, content: reviewsPanel },
          { key: "archived", label: "Archived", count: archivedList.length, content: <SocialThreadList collaboration={collaboration} staff={collabStaff} canCollaborate={canCollaborate} list={archivedList} empty="Nothing archived. Archive finished or test conversations to keep the active queue focused." /> },
        ]}
      />
    </div>
  );
