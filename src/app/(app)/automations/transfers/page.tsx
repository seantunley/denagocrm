          ...
          <table className="table-base">
            ...
            <tbody>
              {rows.map((row) => {
                ...
                return (
                  <tr key={row.id}>
                    <td data-primary data-label="Vehicle / stock unit"><p className="font-medium">{unit?.product?.name ?? "Stock unit"}</p><p className="text-xs text-muted-foreground">{unit?.stockNumber ?? unit?.serial ?? row.stockUnitId}</p>{row.notes && <p className="mt-1 max-w-md text-xs text-muted-foreground">{row.notes}</p>}</td>
                    <td data-label="From">{row.fromBranch ?? "Unassigned"}</td>
                    <td data-label="To">{row.toBranch}</td>
                    <td data-label="Status"><span className={`badge ${row.status === "received" ? "bg-emerald-500/15 text-emerald-300" : row.status === "cancelled" ? "bg-red-500/15 text-red-300" : row.status === "in_transit" ? "bg-blue-500/15 text-blue-300" : "bg-amber-500/15 text-amber-300"}`}>{row.status.replaceAll("_", " ")}</span></td>
                    <td data-label="Requested">{formatDateTime(row.createdAt)}</td>
                    <td data-actions className="min-w-56">
