            ...
            {databaseBackups.map((blob, index) => (
              <div key={blob.pathname} className="flex flex-col gap-3 px-5 py-4 sm:flex-row sm:items-center">
                ...
                <div className="flex items-start justify-end gap-2">
                  ...
                  <Button asChild variant="outline" size="sm"><a href={`/api/backups/file?ref=${encodeURIComponent(blob.url)}`} target="_blank" rel="noreferrer"><ArrowDownToLine className="size-3.5" />Encrypted file</a></Button>
                </div>
              </div>
            ))}
          </div>
