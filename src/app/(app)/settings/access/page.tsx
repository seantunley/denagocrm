        ...
        <section className="space-y-3">
          <div className="flex items-center justify-between gap-3">
            <div>
              ...
              <p className="text-xs text-slate-500">Group members for team-level lead visibility and ownership.</p>
            </div>
            {canManageTeams && (
              <ModalTrigger
          ...
          <div className="card p-0 divide-y divide-slate-800">
            {teams.length === 0 ? (
              <p className="text-sm text-slate-400 p-5">No teams yet{canManageTeams ? " — create one above." : "."}</p>
            ) : (
              teams.map((team) => (
                <details key={team.id}>
                  <summary className="flex items-center justify-between gap-4 px-5 py-4 cursor-pointer select-none list-none [&::-webkit-details-marker]:hidden">
                    <span className="text-sm font-medium flex items-center gap-2 flex-wrap">
                      {team.name}
                      {!team.active && <span className="badge bg-slate-800 text-slate-400">Inactive</span>}
                  ...
                  <div className="px-5 pb-5 space-y-4">
                    {canManageTeams && (
                      <form action={updateTeam.bind(null, team.id)} className="grid sm:grid-cols-2 gap-2">
                        ...
                        <label className="flex items-center gap-2 text-sm text-slate-300"><input type="checkbox" name="active" defaultChecked={team.active} className="h-4 w-4" /> Active</label>
                        <div className="sm:text-right"><button className="btn-secondary btn-sm">Save team</button></div>
                      </form>
                    )}
                    <div>
                      <p className="text-xs uppercase tracking-wide text-slate-500 mb-1">Members</p>
                      <ul className="divide-y divide-slate-800">
                        {membersFor(team.id).length === 0 && <li className="py-2 text-xs text-slate-500">No members yet.</li>}
                        {membersFor(team.id).map((member) => (
                          <li key={member.userId} className="py-2 flex items-center gap-2 text-sm">
                            <span className="flex-1">{member.userName}{member.isManager && <span className="text-xs text-orange-400 ml-2">Manager</span>}</span>
        ...
        <section className="space-y-3">
          <div className="flex items-center justify-between gap-3">
            <div>
              ...
              <p className="text-xs text-slate-500">Bundle permissions into roles, then assign roles to people.</p>
            </div>
            {canManageRoles && (
              <ModalTrigger
              ...
              return (
                <details key={role.id}>
                  <summary className="flex items-center justify-between gap-4 px-5 py-4 cursor-pointer select-none list-none [&::-webkit-details-marker]:hidden">
                    <span className="text-sm font-medium flex items-center gap-2 flex-wrap">
                      {role.name}
                      {role.system && <span className="badge bg-slate-800 text-slate-300">System</span>}
                      <span className="text-xs font-normal text-slate-500">
                        ...
                        <div key={category}>
                          ...
                          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-2">
                            {permissions.filter((p) => p.category === category).map((permission) => (
                              <label key={permission.key} className="rounded-lg border border-slate-800 bg-slate-800/30 p-2 flex gap-2 text-sm">
                                ...
                                <span><strong className="font-medium">{permission.key}</strong><span className="block text-xs text-slate-500">{permission.description}</span></span>
                              </label>
                            ))}
                          </div>
                        </div>
          ...
          <div>
            <p className="text-xs uppercase tracking-wide text-slate-500 mb-1.5">Role assignments</p>
            <div className="card p-0 overflow-x-auto">
              <table className="table-base">
                ...
                <tbody>
                  {users.map((user) => (
                    <tr key={user.id}>
                      <td>{user.name}<p className="text-xs text-slate-500">{user.email}</p>{user.disabledAt && <span className="text-xs text-red-400">Disabled</span>}</td>
                      <td>
