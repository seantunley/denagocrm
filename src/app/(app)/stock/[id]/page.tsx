...
function quoteCustomer(quote: {
  ...
  return quote.contact?.company ?? person || quote.lead?.name || "Customer";
}
...
export default async function StockUnitPage({
  ...
  const [unit, locations, statuses, accessibleLeadIds, accessibleQuoteIds] = await Promise.all([
  ...
  const assigned = await getQuoteStockAssignments(quotes.map((quote) => quote.id));
  const occupiedQuoteIds = new Set(
    assigned.filter((item) => item.stockUnitId !== id).map((item) => item.quoteId),
  );
  const selectableQuotes = quotes.filter((quote) => !occupiedQuoteIds.has(quote.id));
  const statusOption = statuses.find((item) => item.slug === unit.status);
  ...
  const allowed = systemStatus
    ...
    : [];
  const customStatuses = statuses.filter((item) => !item.system);
  const facts: Fact[] = [
  ...
  return (
    <div className="denago-workspace space-y-6">
      ...
      >
        {systemStatus ? (
          <StatusPill tone={STOCK_STATUS_TONES[systemStatus]}>{STOCK_STATUS_LABELS[systemStatus]}</StatusPill>
        ) : (
          <span
            className="rounded-full border px-2.5 py-1 text-xs font-semibold"
            style={{ borderColor: `${statusOption?.color ?? "#64748b"}55`, backgroundColor: `${statusOption?.color ?? "#64748b"}18`, color: statusOption?.color ?? "#94a3b8" }}
          >
            {statusOption?.label ?? unit.status.replaceAll("-", " ")}
          </span>
        )}
      ...
      {unit.reservation && ["expired", "today", "soon"].includes(reservationState) && (
        <FeedbackBanner tone={reservationState === "soon" ? "warning" : "danger"} title="Reservation needs attention">
          Reserved for {unit.reservation.leadName}{unit.reservation.expiresAt ? ` until ${formatDateTime(unit.reservation.expiresAt)}` : " without an expiry"}.
        </FeedbackBanner>
      )}
      ...
      <div className="grid gap-5 xl:grid-cols-[minmax(0,1fr)_24rem]">
        <div className="space-y-5">
          <Surface className="p-5">
            ...
            <div className="mt-5 grid grid-cols-2 gap-2 sm:grid-cols-4 lg:grid-cols-7">
              {["available", "reserved", "allocated", "pdi", "ready", "sold", "delivered"].map((step, index) => {
                const stages = ["available", "reserved", "allocated", "pdi", "ready", "sold", "delivered"];
                ...
                return (
                  <div key={step} className={`rounded-xl border px-3 py-3 text-center ${active ? "border-primary/40 bg-primary/10" : complete ? "border-emerald-400/20 bg-emerald-400/10" : "border-border bg-background/30"}`}>
                    <span className={`mx-auto grid size-7 place-items-center rounded-full ${active ? "bg-primary text-primary-foreground" : complete ? "bg-emerald-400/15 text-emerald-300" : "bg-muted text-muted-foreground"}`}>{complete ? <CheckCircle2 className="size-4" /> : index + 1}</span>
              ...
              <div className="mt-5 space-y-4 border-t border-border pt-5">
                {unit.status === "available" && (
                  <form action={reserveUnit.bind(null, id)} className="rounded-2xl border border-border bg-background/30 p-4">
                    ...
                    <div className="mt-4 grid gap-3 md:grid-cols-4">
                      <select name="leadId" className="input md:col-span-2" required defaultValue=""><option value="" disabled>Select lead…</option>{leads.map((lead) => <option key={lead.id} value={lead.id}>{lead.title} — {lead.name} · {lead.stage.name}</option>)}</select>
                ...
                {unit.status === "reserved" && unit.reservation && (
                  <div className="rounded-2xl border border-amber-400/20 bg-amber-400/10 p-4">
                    ...
                    <div className="mt-4 flex flex-wrap gap-2">
                      ...
                      {unit.reservation.depositRequiredCents > 0 && !unit.reservation.depositReceivedAt && <form action={recordReservationDeposit.bind(null, id)}><button className="btn-primary btn-sm">Record {formatZAR(unit.reservation.depositRequiredCents)} deposit</button></form>}
                      <ModalTrigger label="Release reservation" title="Release this stock reservation" buttonClass="btn-secondary btn-sm"><form action={releaseUnit.bind(null, id)} className="card space-y-3"><textarea name="reason" className="input min-h-24" required placeholder="Why is this reservation being released?" /><button className="btn-danger w-full">Release reservation</button></form></ModalTrigger>
                    </div>
                  </div>
                )}
                ...
                {["available", "reserved"].includes(unit.status) && selectableQuotes.length > 0 && (
                  <form action={allocateUnitToQuote.bind(null, id)} className="rounded-2xl border border-sky-400/20 bg-sky-400/10 p-4">
                    ...
                    <div className="mt-4 flex flex-col gap-3 sm:flex-row"><select name="quoteId" className="input flex-1" required defaultValue=""><option value="" disabled>Select accepted quote…</option>{selectableQuotes.map((quote) => <option key={quote.id} value={quote.id}>Q-{quote.number} — {quoteCustomer(quote)}</option>)}</select><button className="btn-primary">Allocate unit</button></div>
                  </form>
                )}

                {unit.status === "allocated" && <form action={beginPdi.bind(null, id)} className="rounded-2xl border border-primary/20 bg-primary/10 p-4"><div className="flex items-center justify-between gap-3"><div><p className="font-semibold">Start pre-delivery inspection</p><p className="mt-1 text-xs text-muted-foreground">Moves the unit into the preparation queue.</p></div><button className="btn-primary"><Wrench className="size-4" /> Start PDI</button></div></form>}
                ...
                {unit.status === "pdi" && (
                  <form action={completePdi.bind(null, id)} className="rounded-2xl border border-border bg-background/30 p-4">
                    ...
                    <div className="mt-4 grid gap-2 sm:grid-cols-2 lg:grid-cols-4">{[["battery", "Battery & terminals"], ["charger", "Charger & cable"], ["tyres", "Tyres & pressure"], ["brakes", "Brakes"], ["lights", "Lights & indicators"], ["body", "Body & trim"], ["keys", "Keys / remotes"], ["roadTest", "Road test"]].map(([name, label]) => <label key={name} className="flex items-center gap-2 rounded-xl border border-border bg-card/60 px-3 py-2.5 text-sm"><input type="checkbox" name={name} required /> {label}</label>)}</div>
                    <textarea name="notes" className="input mt-3 min-h-20" placeholder="PDI observations" />
                    <button className="btn-primary mt-3"><CheckCircle2 className="size-4" /> Complete PDI and mark ready</button>
                  </form>
                )}

                {unit.status === "ready" && <FeedbackBanner tone="success" title="Ready for delivery">Complete handover from the Deliveries board; stock, warranty and vehicle registration will close atomically.</FeedbackBanner>}

                {allowed.length > 0 && <ModalTrigger label={<><PauseCircle className="size-4" /> Change operational state</>} title="Controlled stock status change" buttonClass="btn-secondary btn-sm"><form action={changeStockStatus.bind(null, id)} className="card space-y-3"><select name="toStatus" className="input" required defaultValue=""><option value="" disabled>Select status…</option>{allowed.map((target) => <option key={target} value={target}>{STOCK_STATUS_LABELS[target]}</option>)}</select><textarea name="reason" className="input min-h-24" required placeholder="Reason" /><button className="btn-primary w-full">Apply status change</button></form></ModalTrigger>}

                {customStatuses.length > 0 && !COMMITTED_STATUSES.has(unit.status) && (
                  <ModalTrigger label="Apply custom label" title="Apply organisational stock status" buttonClass="btn-secondary btn-sm">
                    <form action={setStockStatus.bind(null, id)} className="card space-y-3"><select name="status" className="input" required defaultValue=""><option value="" disabled>Select custom status…</option>{customStatuses.map((option) => <option key={option.slug} value={option.slug}>{option.label}</option>)}</select><textarea name="reason" className="input min-h-24" required placeholder="Why is this organisational label being applied?" /><button className="btn-primary w-full">Apply custom status</button></form>
                  </ModalTrigger>
                )}
              </div>
            )}
          ...
          <Surface className="p-5">
            <SectionHeading title="Unit identity and condition" description="Manufacturer identifiers, equipment, operating data and storage context." action={<Fingerprint className="size-5 text-primary" />} />
            <form action={updateStockUnit.bind(null, id)} className="mt-5 space-y-4">
              <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-3">
                ...
                <div><label className="label">Battery type</label><input name="batteryType" className="input" defaultValue={unit.batteryType ?? ""} /></div>
              ...
              {canManage && <div><label className="label">Adjustment reason</label><input name="reason" className="input" placeholder="Context for cost or identity changes" /></div>}
              {canManage && <button className="btn-secondary">Save unit details</button>}
            </form>
          </Surface>

          <Surface className="p-5">
            <SectionHeading title="Photos and documents" description="Condition, serial plates, accessories, PDI evidence and supplier paperwork." action={<Camera className="size-5 text-primary" />} />
            {canManage && <form action={uploadStockFiles.bind(null, id)} className="mt-4 flex flex-col gap-3 rounded-2xl border border-dashed border-border bg-background/30 p-4 sm:flex-row sm:items-end"><div className="flex-1"><label className="label">Files</label><input type="file" name="files" multiple accept="image/*,.pdf" className="block w-full text-xs text-muted-foreground file:btn-secondary file:btn-sm file:mr-2 file:border-0" /></div><select name="category" className="input sm:w-44" defaultValue="photo"><option value="photo">Photo</option><option value="serial">Serial plate</option><option value="condition">Condition report</option><option value="pdi">PDI evidence</option><option value="supplier">Supplier document</option></select><button className="btn-primary">Upload</button></form>}
            <div className="mt-4 grid gap-3 sm:grid-cols-2 xl:grid-cols-3">{unit.attachments.map((attachment) => <a key={attachment.id} href={attachment.storedName} target="_blank" rel="noreferrer" className="rounded-2xl border border-border bg-background/30 p-4 transition-colors hover:border-primary/30"><p className="truncate text-sm font-medium text-foreground">{attachment.fileName}</p><p className="mt-1 text-xs text-muted-foreground">{attachment.category} · {(attachment.sizeBytes / 1024).toFixed(0)} KB</p></a>)}{unit.attachments.length === 0 && <p className="text-sm text-muted-foreground">No files added yet.</p>}</div>
          </Surface>

          <Surface className="p-5">
            <SectionHeading title="Movement ledger" description="Append-only receiving, reservation, allocation, PDI, value and delivery history." action={<History className="size-5 text-primary" />} />
            <ol className="relative mt-5 space-y-4 border-l border-border pl-5">{unit.movements.map((movement) => <li key={movement.id} className="relative rounded-2xl border border-border bg-background/30 p-4 before:absolute before:-left-[25px] before:top-5 before:size-2 before:rounded-full before:bg-primary before:ring-4 before:ring-card"><div className="flex flex-wrap items-start justify-between gap-2"><div><p className="font-medium capitalize text-foreground">{movement.eventType.replaceAll("_", " ")}</p><p className="mt-1 text-xs text-muted-foreground">{movement.fromStatus ? `${movement.fromStatus} → ` : ""}{movement.toStatus ?? "record updated"}</p></div><p className="text-[10px] text-muted-foreground">{formatDateTime(movement.occurredAt)}</p></div>{movement.reason && <p className="mt-2 text-sm text-muted-foreground">{movement.reason}</p>}<p className="mt-3 text-[10px] text-muted-foreground">{movement.performedByName}{movement.costAfterCents != null ? ` · ${formatZAR(movement.costAfterCents)}` : ""}</p></li>)}</ol>
          </Surface>
        </div>

        <aside className="space-y-5 xl:sticky xl:top-6 xl:self-start">
          <Surface className="p-5"><SectionHeading title="Unit facts" description="Operational identifiers at a glance." /><dl className="mt-4 space-y-3 text-sm">{facts.map(({ label, value, icon: Icon }) => <div key={label} className="flex items-start gap-3 rounded-xl border border-border bg-background/30 p-3"><Icon className="mt-0.5 size-4 shrink-0 text-muted-foreground" /><div className="min-w-0"><dt className="text-[10px] font-semibold uppercase tracking-[0.1em] text-muted-foreground">{label}</dt><dd className="mt-1 break-words font-medium text-foreground">{value}</dd></div></div>)}</dl></Surface>
          {unit.purchaseOrderReference && <Surface className="p-5" inset><SectionHeading title="Supplier origin" description="Received through purchasing." /><p className="mt-4 text-sm font-medium text-foreground">{unit.purchaseOrderReference}</p><Link href="/stock/purchase-orders" className="mt-3 inline-flex items-center gap-2 text-xs font-medium text-primary hover:underline"><Truck className="size-3.5" /> Open purchase orders</Link></Surface>}
          {unit.status === "delivered" && <Surface className="border-emerald-400/20 bg-emerald-400/5 p-5"><SectionHeading title="Delivery complete" description="Customer vehicle created automatically at handover." action={<CheckCircle2 className="size-5 text-emerald-300" />} />{unit.warrantyExpiresAt && <p className="mt-4 text-xs text-emerald-100/70">Warranty through {unit.warrantyExpiresAt.toLocaleDateString("en-ZA")}</p>}</Surface>}
          {canManage && !COMMITTED_STATUSES.has(unit.status) && <Surface className="border-red-400/20 p-5"><SectionHeading title="Archive unit" description="Remove from active stock while retaining the ledger." action={<AlertTriangle className="size-5 text-red-300" />} /><form action={deleteStockUnit.bind(null, id)} className="mt-4 space-y-3"><textarea name="reason" required className="input min-h-20" placeholder="Why is this unit leaving active stock?" /><button className="btn-danger w-full">Archive stock unit</button></form></Surface>}
        </aside>
      </div>
    </div>
  );
