...
function isAllDayActivity(date: Date): boolean {
...
export default async function ActivitiesPage({
  ...
  const overdue = activities.filter((activity) =>
    isAllDayActivity(activity.dueDate)
      ? activity.dueDate < startOfToday
      : activity.dueDate < now,
  );
  const overdueIds = new Set(overdue.map((activity) => activity.id));
  const today = activities.filter(
    (activity) =>
      !overdueIds.has(activity.id) && activity.dueDate < endOfToday,
  );
  const upcoming = activities.filter(
    (activity) => activity.dueDate >= endOfToday,
  );
  const groups = [
    {
    ...
    <div className="space-y-6">
      ...
      <section className="rounded-2xl border border-border bg-card/70 p-3 shadow-sm">
        <div className="flex flex-col gap-3 xl:flex-row xl:items-center xl:justify-between">
          ...
          <form
            action="/activities"
            className="flex min-w-0 flex-1 flex-col gap-2 sm:flex-row xl:max-w-3xl"
            role="search"
          >
            ...
            <select
              name="type"
              defaultValue={type ?? ""}
              className="input h-10 rounded-xl bg-background/50 sm:w-40"
            >
            ...
            {filtersActive && (
              <Link
                href={mine ? "/activities" : "/activities?who=all"}
                  ...
                  <div className="divide-y divide-border/70">
                    {group.items.map((activity) => {
                      ...
                      return (
                        <article
                          key={activity.id}
                          className="group relative px-4 py-4 transition-colors hover:bg-white/[0.025] sm:px-5"
                        >
                          <div className="flex items-start gap-3.5">
                            ...
                            <div className="min-w-0 flex-1">
                              <div className="flex flex-wrap items-start justify-between gap-x-4 gap-y-1">
                                ...
                                <StatusPill
                                  tone={
                                    group.tone === "neutral" ? "neutral" : group.tone
                                  }
                                >
                                  {group.title}
                                </StatusPill>
                                ...
                                {canManage && (
                                  <div className="flex items-center gap-2">
                                    ...
                                    >
                                      ...
                                      <input
                                        name="note"
                                        className="input h-8 min-w-0 flex-1 text-xs sm:w-40 xl:w-48"
