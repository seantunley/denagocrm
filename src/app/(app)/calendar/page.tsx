...
export default async function CalendarPage({
  ...
  const activities = await prisma.activity.findMany({
    where: { dueDate: { gte: gridStart, lt: gridEnd }, status: "planned" },
    ...
    <div className="space-y-5">
      <div className="flex items-center justify-between gap-4 flex-wrap">
        ...
        <div className="flex gap-2">
          <Link href={`/calendar?m=${prev}`} className="btn-secondary">
            ← {format(addMonths(monthStart, -1), "MMM")}
          </Link>
          <Link href="/calendar" className="btn-secondary">
            Today
          </Link>
          <Link href={`/calendar?m=${next}`} className="btn-secondary">
            {format(addMonths(monthStart, 1), "MMM")} →
          </Link>
        </div>
      </div>
      ...
      <div className="card p-0 overflow-hidden">
        <div className="grid grid-cols-7 border-b border-slate-800">
          {["Mon", "Tue", "Wed", "Thu", "Fri", "Sat", "Sun"].map((d) => (
            <div key={d} className="px-2 py-2 text-xs font-semibold uppercase tracking-wide text-slate-400 text-center">
              {d}
            </div>
          ))}
        </div>
      ...
      <p className="text-xs text-slate-500">
        Shows planned activities and demos. Schedule them from any lead or contact — meetings show
        in orange, overdue in red.
      </p>
    </div>
  );
}
