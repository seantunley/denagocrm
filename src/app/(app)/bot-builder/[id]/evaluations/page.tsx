...
import { CheckCircle2, FlaskConical, ListChecks, Play, Plus, Trash2, XCircle } from "lucide-react";
...
export default async function FlowEvaluationsPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const passed = evaluations.filter((evaluation) => evaluation.lastStatus === "passed").length;
    ...
    <EntityDetailShell
      ...
      actions={evaluations.length ? (
        <SaveForm action={runAllFlowEvaluations.bind(null, id)} success="Evaluation suite finished">
          <SaveButton className="btn-primary btn-sm" pendingLabel="Running suite…"><Play className="size-4" />Run all</SaveButton>
        </SaveForm>
      ) : undefined}
    >
      <div className="space-y-5">
        <div className="grid gap-3 sm:grid-cols-3">
          ...
          <Metric label="Passing" value={passed} tone="text-emerald-300" />
          <Metric label="Needs attention" value={evaluations.filter((evaluation) => ["failed", "error"].includes(evaluation.lastStatus)).length} tone="text-amber-300" />
        </div>
        ...
        {evaluations.length === 0 ? (
          ...
          <div className="space-y-3">
            {evaluations.map((evaluation) => {
              ...
              return (
                <Surface key={evaluation.id} className="p-5">
                  <div className="flex flex-wrap items-start justify-between gap-3">
                    ...
                    <div className="flex gap-2">
                      <SaveForm action={runFlowEvaluation.bind(null, evaluation.id)} success="Evaluation finished"><SaveButton className="btn-secondary btn-sm" pendingLabel="Running…"><Play className="size-3.5" />Run</SaveButton></SaveForm>
                  ...
                  {evaluation.lastRunAt && (
                    <div className={`mt-4 rounded-xl border p-3 ${evaluation.lastStatus === "passed" ? "border-emerald-400/15 bg-emerald-500/5" : "border-amber-400/15 bg-amber-500/5"}`}>
                      <div className="flex items-center gap-2 text-xs font-medium">{evaluation.lastStatus === "passed" ? <CheckCircle2 className="size-4 text-emerald-400" /> : <XCircle className="size-4 text-amber-400" />}Last run {evaluation.lastRunAt.toLocaleString("en-ZA")} · outcome {result.outcome ?? "error"}{result.finalNodeId ? ` · waiting at ${result.finalNodeId}` : ""}</div>
