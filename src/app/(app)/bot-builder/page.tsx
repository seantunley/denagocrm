...
export default async function BotBuilderPage() {
  ...
  if ((await prisma.botFlow.count()) === 0) {
    ...
    // An old custom flow may predate today's compiler/channel rules. Keep it as
    // the compatibility active flow rather than making the library page unusable;
    // its lint panel will explain what must be fixed before the first publication.
          ...
          <Surface key={f.id} className="group flex min-h-52 flex-col p-5 transition hover:border-primary/35">
            <div className="flex items-center justify-between gap-2">
              <form action={renameFlow.bind(null, f.id)} className="flex-1 min-w-0">
                <input name="name" defaultValue={f.name} className="bg-transparent font-semibold text-foreground w-full outline-none focus:bg-muted rounded px-1 -mx-1" />
              </form>
            ...
            {(blocked || check.warnings.length > 0) && (
              <p className={`mt-2 text-xs ${blocked ? "text-red-300" : "text-amber-300"}`}>
                {blocked
                  ? check.errors[0].message
                  : `${check.warnings.length} warning${check.warnings.length === 1 ? "" : "s"} — review before publishing.`}
              </p>
            )}
            ...
            <div className="flex gap-2 flex-wrap border-t border-border/70 pt-3">
              ...
              {(!f.active || pending) && !blocked && (
                <form action={setActiveFlow.bind(null, f.id)}>
                  <button className="btn-secondary btn-sm">{f.active ? "Publish changes" : "Publish"}</button>
                </form>
              )}
              {(!f.active || pending) && blocked && (
                <span className="btn-secondary btn-sm cursor-not-allowed opacity-50" title="Fix publish errors first">Publish blocked</span>
              )}
