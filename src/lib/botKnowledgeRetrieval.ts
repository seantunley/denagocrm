/**
 * Parsing, validity and retrieval for owner-curated chatbot knowledge.
 *
 * Kept import-free and split out of `botKnowledge.ts` (which is `server-only`
 * and reads settings) so the rules below can be EXECUTED by a test instead of
 * being asserted with a regex over the source. Same shape as `botOwnership.ts`
 * and `botBookingIdentity.ts`: the boundary lives in a module a test can call.
 */
 ...
 *
 * The comparator is TOTAL on purpose. `score` then `updatedAt` alone leaves ties
 * to fall through to the stored array order, so two approved entries that
 * contradict each other — same score, same timestamp — would be ranked by
 * wherever they happened to sit in the BOT_KNOWLEDGE_ENTRIES JSON. Editing an
 * unrelated entry could then silently change which contradicting fact leads the
 * prompt, and which one `renderKnowledgeForPrompt` drops when it hits maxChars.
 * `id` is the last resort so the ranking depends only on the entries themselves.
 */
