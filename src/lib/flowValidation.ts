...
const NODE_TYPES = new Set(["message", "choice", "capture", "captureFile", "image", "answer", "booking", "slots", "condition", "ai", "handoff", "end"]);
const BUILTIN_VARS = new Set(["greeting", "first_name", "name", "known", "slot", "channel", "current_date", "current_time"]);
...
function refs(node: FlowNode): string[] {
  ...
  const next = str((node as { next?: unknown }).next); return next ? [next] : [];
}
...
function reachable(flow: Flow): Set<string> { const seen = new Set<string>(); const stack = [flow.start]; while (stack.length) { const id = stack.pop()!; if (seen.has(id)) continue; const node = flow.nodes[id]; if (!node || typeof node !== "object") continue; seen.add(id); for (const target of refs(node as FlowNode)) stack.push(target); } return seen; }
function automaticCycles(flow: Flow): string[][] {
  const visiting = new Set<string>(), visited = new Set<string>(), stack: string[] = [], cycles: string[][] = []; const emitted = new Set<string>();
  function visit(id: string) { if (visiting.has(id)) { const start = stack.indexOf(id); const cycle = stack.slice(start).concat(id); const key = [...new Set(cycle)].sort().join("|"); if (!emitted.has(key)) { emitted.add(key); cycles.push(cycle); } return; } if (visited.has(id)) return; const node = flow.nodes[id] as FlowNode | undefined; if (!node || !NODE_TYPES.has(node.type) || !AUTO_TYPES.has(node.type)) return; visiting.add(id); stack.push(id); for (const target of refs(node)) { const targetNode = flow.nodes[target] as FlowNode | undefined; if (targetNode && NODE_TYPES.has(targetNode.type) && AUTO_TYPES.has(targetNode.type)) visit(target); } stack.pop(); visiting.delete(id); visited.add(id); }
  for (const id of Object.keys(flow.nodes)) visit(id); return cycles;
}
...
export function flowVariables(flow: Flow): string[] { const vars = new Set(BUILTIN_VARS); for (const node of Object.values(flow.nodes)) if (node?.type === "capture" || node?.type === "captureFile") { const variable = str(node.variable); if (variable) vars.add(variable); } return [...vars].sort(); }
...
export function validateFlow(flow: Flow, channels: FlowChannel[] = ["whatsapp"]): FlowIssue[] {
  ...
  const entries = Object.entries(flow.nodes); if (!str(flow.start) || !flow.nodes[str(flow.start)]) return [issue("error", "graph.start", "Flow needs a valid start node.")]; if (!entries.length) return [issue("error", "graph.empty", "Flow has no nodes.")]; if (entries.length > 250) issues.push(issue("error", "graph.size", "Flow exceeds the 250-node safety limit."));
  ...
  for (const [key, rawNode] of entries) {
    ...
    const runtimeNode = rawNode as Record<string, unknown>; const id = str(runtimeNode.id); const type = str(runtimeNode.type); if (!id || !NODE_TYPES.has(type)) { issues.push(issue("error", "node.shape", `Node “${key}” has an unsupported or malformed type.`, key)); continue; } const node = rawNode as FlowNode; if (id !== key) issues.push(issue("error", "node.id_mismatch", `Node key “${key}” does not match its id “${id}”.`, key));
    if (node.type === "capture" || node.type === "captureFile") { const variable = str(node.variable); if (!variable || !/^\w+$/.test(variable)) issues.push(issue("error", "variable.invalid", "Captured variable names must contain only letters, numbers or underscores.", id)); else { if (captured.has(variable) && !BUILTIN_VARS.has(variable)) issues.push(issue("warning", "variable.reused", `Variable {{${variable}}} is captured more than once.`, id)); captured.add(variable); } }
    if (node.type === "choice") { const rawOptions = runtimeNode.options; const options = optionsOf(node); if (!Array.isArray(rawOptions) || !options.length) issues.push(issue("error", "choice.empty", "Menu needs at least one valid option.", id)); else { const ids = new Set<string>(); for (const option of options) { if (!option.id || ids.has(option.id)) issues.push(issue("error", "choice.option_id", "Menu option ids must be non-empty and unique within the node.", id)); ids.add(option.id); if (!option.label.trim()) issues.push(issue("error", "choice.label", "Every menu option needs a label.", id)); if (!option.next) issues.push(issue("warning", "choice.dead_end", `Option “${option.label || option.id}” ends the conversation without an explicit End node.`, id)); } for (const channel of channels) issues.push(...validateChoiceForChannel(id, options, channel)); } }
    if (node.type === "condition") { const variable = str(node.condition?.variable); const operator = str(node.condition?.operator); if (!variable || !/^\w+$/.test(variable)) issues.push(issue("error", "condition.variable", "Condition needs a valid variable name.", id)); if (!["equals", "not_equals", "contains", "exists", "empty"].includes(operator)) issues.push(issue("error", "condition.operator", "Condition uses an unsupported operator.", id)); if (["equals", "not_equals", "contains"].includes(operator) && !str(node.condition?.value).trim()) issues.push(issue("warning", "condition.value", "This condition compares against an empty value.", id)); if (!node.trueNext) issues.push(issue("warning", "condition.true_dead_end", "Condition has no Yes/true destination.", id)); if (!node.falseNext) issues.push(issue("warning", "condition.false_dead_end", "Condition has no No/false destination.", id)); }
    ...
    if (node.type === "captureFile") for (const channel of channels) if (channel !== "whatsapp") { const label = channel === "instagram" ? "Instagram" : channel === "messenger" ? "Messenger" : "Telegram"; issues.push(issue("error", "channel.file_capture", `File capture is not implemented for ${label}; this flow would pause forever on that channel.`, id, channel)); }
  }
  const live = reachable(flow); for (const [id] of entries) if (!live.has(id)) issues.push(issue("warning", "graph.unreachable", "Node is not reachable from the start node.", id)); for (const cycle of automaticCycles(flow)) issues.push(issue("error", "graph.automatic_cycle", `Automatic loop detected: ${cycle.join(" → ")}. It would run until the engine guard stops it.`, cycle[0]));
  for (const [, rawNode] of entries) { if (!rawNode || typeof rawNode !== "object" || !NODE_TYPES.has(str((rawNode as { type?: unknown }).type))) continue; const node = rawNode as FlowNode; for (const text of allText(node)) for (const variable of referencedVars(text)) if (!captured.has(variable)) issues.push(issue("warning", "variable.unknown", `Message references {{${variable}}}, but no capture or built-in variable defines it.`, str(node.id))); if (node.type === "condition") { const variable = str(node.condition?.variable); if (variable && !captured.has(variable)) issues.push(issue("warning", "condition.unknown_variable", `Condition checks {{${variable}}}, but no capture or built-in variable defines it.`, str(node.id))); } }
  return issues;
}
