...
function validateChoiceForChannel(
  ...
  if (channel === "telegram") {
    for (const option of options) {
      if (encodedBytes(choiceId(nodeId, option.id)) > 64) out.push(issue("error", "channel.telegram.callback", "Telegram callback data is limited to 64 bytes; this option id would be truncated and stop matching.", nodeId, channel));
    }
  }
  return out;
}
