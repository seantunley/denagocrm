...
export type FlowSnapshot = { flow: Flow; versionId: string | null; flowId: string | null };
...
function parseFlow(definition: string): Flow | null {
  try { const parsed = JSON.parse(definition); if (parsed?.start && parsed?.nodes?.[parsed.start]) return parsed as Flow; } catch { /* safe fallback */ }
  return null;
}

export async function resolveFlowSnapshot(channel: string, pinnedVersionId?: string | null): Promise<FlowSnapshot> {
  if (pinnedVersionId) {
  ...
  const publication = (await prisma.botFlowPublication.findFirst({ where: { channel } })) ?? (channel === "whatsapp" ? null : await prisma.botFlowPublication.findFirst({ where: { channel: "whatsapp" } }));
  if (publication) { const version = await prisma.botFlowVersion.findUnique({ where: { id: publication.versionId } }); const flow = version ? parseFlow(version.definition) : null; if (version && flow) return { flow, versionId: version.id, flowId: version.flowId }; }
  const legacy = (await prisma.botFlow.findFirst({ where: { channel, active: true } })) ?? (channel === "whatsapp" ? null : await prisma.botFlow.findFirst({ where: { channel: "whatsapp", active: true } }));
  if (legacy) { const flow = parseFlow(legacy.definition); if (flow) return { flow, versionId: null, flowId: legacy.id }; }
  return { flow: DEFAULT_FLOW, versionId: null, flowId: null };
}

export async function publishFlowSnapshot(flowId: string, actorId?: string | null): Promise<{ versionId: string; version: number; channel: string }> {
  ...
  if (!parsed) throw new FlowPublishValidationError([{ severity: "error", code: "graph.shape", message: "Flow definition is malformed." }]);
  ...
  return withTenantWrite(async (tx, tenantId) => {
    ...
    const snapshot = await tx.botFlowVersion.create({ data: { tenantId, flowId, channel: flow.channel, version, definition: flow.definition, createdById: actorId ?? null } });
    ...
    await tx.botFlowPublication.upsert({ where: { tenantId_channel: { tenantId, channel: flow.channel } }, update: { flowId: flow.id, versionId: snapshot.id, publishedById: actorId ?? null, publishedAt: new Date() }, create: { tenantId, channel: flow.channel, flowId: flow.id, versionId: snapshot.id, publishedById: actorId ?? null } });
