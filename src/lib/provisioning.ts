 ...
 * `disabledAt` on the provisioned owner (raw SQL — `disabledAt` is the security
 * column managed outside the Prisma model), so the owner can finally sign in.
 *
 * `ownerId` MUST be the account {@link createTenant} provisioned — the caller
 * resolves it from `Tenant.ownerUserId`, or from the earliest TenantMember for
 * tenants created before that column existed. Only that specific account is
 * re-enabled: other TenantMembers are existing platform users whose global
 * `disabledAt` state is unrelated to this tenant's inert setup and must not be
 * touched — a security suspension on a shared account must survive a second
 * tenant's activation.
 *
