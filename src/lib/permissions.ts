...
/**
 * Once Role exists, RBAC is authoritative. Query/storage failures fail closed.
 * Owners retain an explicit application-level bypass for emergency governance.
 */
...
export async function hasAnyPermission(
  ...
  if (granted.has(RBAC_UNAVAILABLE) || !granted.has(RBAC_INITIALIZED)) return false;
...
async function scopedPermissions(user: PermissionUser): Promise<Set<string> | null> {
...
function hasAllScope(permissions: Set<string> | null, key: PermissionKey): boolean {
  return permissions === null || permissions.has(key);
}

function hasOwnedScope(permissions: Set<string> | null, key: PermissionKey): boolean {
  return permissions === null || permissions.has(key);
}
...
/** Null means unrestricted; an array is the complete accessible record set. */
export async function getAccessibleLeadIds(user: PermissionUser): Promise<string[] | null> {
  ...
  const rows = await basePrisma.lead.findMany({
    where: {
      deletedAt: null,
      OR: [
...
export async function getAccessibleContactIds(user: PermissionUser): Promise<string[] | null> {
  const permissions = await scopedPermissions(user);
  if (hasAllScope(permissions, "contacts.view_all")) return null;
  if (!hasOwnedScope(permissions, "contacts.view_owned")) return [];
  ...
  const rows = await basePrisma.contact.findMany({
    where: {
      ...
      OR: [
        ...
        { jobCards: { some: { deletedAt: null, technicianId: user.id } } },
      ],
    },
    select: { id: true },
  });
...
export async function getAccessibleQuoteIds(user: PermissionUser): Promise<string[] | null> {
  const permissions = await scopedPermissions(user);
  if (hasAllScope(permissions, "quotes.view_all")) return null;
  if (!hasOwnedScope(permissions, "quotes.view_owned")) return [];
...
export async function getAccessibleVehicleIds(user: PermissionUser): Promise<string[] | null> {
  const permissions = await scopedPermissions(user);
  if (hasAllScope(permissions, "vehicles.view_all")) return null;
  if (!hasOwnedScope(permissions, "vehicles.view_owned")) return [];
  ...
  const rows = await basePrisma.vehicle.findMany({
    where: {
      ...
      OR: [
        ...
        ...(contactIds.length ? [{ fleet: { contactId: { in: contactIds } } }] : []),
...
export async function getAccessibleJobCardIds(user: PermissionUser): Promise<string[] | null> {
  const permissions = await scopedPermissions(user);
  if (hasAllScope(permissions, "jobcards.view_all")) return null;
  if (!hasOwnedScope(permissions, "jobcards.view_owned")) return [];
...
export async function getAccessibleDocumentIds(user: PermissionUser): Promise<string[] | null> {
  const permissions = await scopedPermissions(user);
  if (hasAllScope(permissions, "documents.view_all")) return null;
  if (!hasOwnedScope(permissions, "documents.view_owned")) return [];
  ...
  if (contactIds === null || quoteIds === null || vehicleIds === null || jobCardIds === null) {
    return null;
  }
...
export async function requireDocumentReadAccess(documentId: string): Promise<PermissionUser> {
  ...
  if (!(await canAccessDocument(user, documentId))) redirect("/");
  return user;
}
...
/* ── Customer-case scopes ────────────────────────────────────────── */

export async function getAccessibleCaseIds(user: PermissionUser): Promise<string[] | null> {
  const permissions = await scopedPermissions(user);
  if (hasAllScope(permissions, "cases.view_all")) return null;
  if (!hasOwnedScope(permissions, "cases.view_owned")) return [];
  const [contactIds, vehicleIds] = await Promise.all([
    getAccessibleContactIds(user),
    getAccessibleVehicleIds(user),
  ]);
