...
**Pre-event receive-side reads → system scope (review blocker 1, fixed).** The webhook's signature-verification reads — `getSetting("META_APP_SECRET"/"META_VERIFY_TOKEN")`, in both the GET handshake and the POST — run BEFORE the per-event chokepoint, so under enforcement a bare (tenant-scoped) `AppSetting` read would throw and verification would never complete. These are install-global (one Meta app, one webhook endpoint), so both routes now read them through `validateInSystemScope(() => getSetting(...))` (the trusted pre-principal boundary; dormant → a bare call, no overhead). Proven enforcement-on: a bare read fails closed, the `validateInSystemScope` wrapper succeeds.

> **Still a prerequisite (not C3):** the *send-side* settings (`WA_PHONE_NUMBER_ID`, `WA_ACCESS_TOKEN`, `META_PAGE_ACCESS_TOKEN`) are read INSIDE the tenant scope and become **per-tenant** once `AppSetting` is tenant-scoped (its slice). Until then those reads (e.g. leadgen's `META_PAGE_ACCESS_TOKEN`) are covered by the same AppSetting/enforcement gate. Dormant today, no effect yet.
...
| **C4 (cron/queues)** ✅ | `automations.ts`, `journeyStepExecutor.ts`, `lifecycleJourneys.ts`, `serviceReminders.ts` (×2), `reviewRequests.ts`, `signingReminders.ts`, `imapSync.ts`, `surveys.ts` (deliverInvite), `cron/automations/route.ts` (stock owner, `ownerOnly`) | queue/reminder attribution |
| **staff/portal (already scoped)** ✅ | `app/actions/warranty.ts`, `app/actions/portal.ts` (`firstStaffUser`) | swapped to `resolveTenantActor` in C4 |

> **C1** rows were fixed in that slice (public token surfaces); **C3** rows in C3 (inbound channel scope); **C4** rows — every cron/queue pick plus the two staff/portal picks — are now swapped to `resolveTenantActor` (each runs inside the scope its cron establishes). Only the pure-staff request-scoped picks outside this table remain on the plain global pick, which is correct (they run inside an authenticated staff scope). The earlier "three owner picks" note was wrong — this table is the source of truth.
...
The per-tenant-loop branch is **dead code until the flip** — scaffolded now, exercised in the enforcement PR. `resolveTenantActor` replaces the global owner pick inside each per-tenant slice.

**✅ Built (C4).** `withSystemScope(fn)` (`tenantScopeEntry.ts`, sibling of `validateInSystemScope`) wraps the backup + security routes — the whole-DB export and the security runbook + admin push are platform-global (guard bypass); dormant → a bare call. `runCronPerTenant(slice)` (`tenantCron.ts`) is the per-tenant scaffold: dormant runs the slice **once** unscoped (byte-for-byte the existing global sweep), enforcing runs it once per `activeTenantIds()` inside each tenant's `runInTenantScope`. The journeys / automations / competitor-watch routes call it, keeping their module gate inside the slice (per-tenant under enforcement) and unwrapping the single dormant run to preserve the exact pre-tenancy response shape. The automations route keeps its **platform-global maintenance** (AI health, backup watchdog, `ErrorLog` 30-day retention — `ErrorLog` is a global model) in a single `withSystemScope` block *outside* the per-tenant loop, so it isn't repeated per tenant. Inbound-email filing (`syncInboundEmail`) rides the outer per-tenant loop — under enforcement each tenant's mailboxes sync in that tenant's scope, subsuming the per-`SupportMailbox` scoping. Integration coverage: a bare guarded sweep fails closed while `withSystemScope` sees both tenants; `runCronPerTenant` confines each tenant's slice and skips suspended tenants; dormant runs once globally.
...
| **C4** ✅ | **Cron scoping** — `withSystemScope` for backup + security (guard bypass for whole-DB sweep + platform maintenance incl. the once-only `ErrorLog` retention); `runCronPerTenant` scaffold (`tenantCron.ts`) for journeys/automations/competitor-watch (dormant → one global run, enforcing → one scoped run per active tenant); **11 cron/queue + staff/portal actor picks → `resolveTenantActor`**; structural + enforcement-on integration tests. | none | none | Med — touches the cron routes + queue actor picks (dormant branch is byte-for-byte; per-tenant branch is dead code until the flip). |
...
5. **HTTP-level booking test** (non-blocking C2 follow-up): assert the *persisted* tenant IDs on `Contact`/`JobCard`/`Activity` by invoking the real `bookings` POST end-to-end, replacing the current structural + `claimSlotCapacity`-level coverage. ~~Blocked on the `AppSetting.tenantId` slice~~ **UNBLOCKED** — the `AppSetting.tenantId` slice landed (additive nullable column + index + backfill to `tenant_denago_cpt`, migration `20260723120000`; `AppSetting` cleared from the schema-contract `PENDING` set — every model is now global or tenant-scoped). Under enforcement the route's `getSlotConfig()` → `getSetting()` now scopes to the request tenant via the guard instead of 500-ing on a missing column; install-global reads (`META_APP_SECRET`, verify token, backup/security config) use a system-scope bypass. `key` stays the `@id` (one row per key) — per-tenant values for the SAME key are deferred to the uniqueness batch. This HTTP-level test can now run enforcement-on and is the next follow-up.
