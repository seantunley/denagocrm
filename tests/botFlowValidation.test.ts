...
test("the shipped default flow compiles for every currently supported text channel", () => {
  assert.deepEqual(flowErrors(validateFlow(DEFAULT_FLOW, ["whatsapp", "messenger", "instagram", "telegram"])), []);
});
...
test("publication rejects missing targets and automatic no-input loops", () => {
  ...
  const loop: Flow = { start: "a", nodes: { a: { id: "a", type: "message", text: "A", next: "b" }, b: { id: "b", type: "answer", text: "B", next: "a" } } };
...
test("conditions participate in graph reachability and automatic-loop safety", () => {
  const flow: Flow = { start: "gate", nodes: { gate: { id: "gate", type: "condition", condition: { variable: "channel", operator: "equals", value: "whatsapp" }, trueNext: "yes", falseNext: "no" }, yes: { id: "yes", type: "message", text: "Yes", next: "end" }, no: { id: "no", type: "message", text: "No", next: "end" }, end: { id: "end", type: "end" } } };
...
test("condition variables are checked against built-ins and captured values", () => {
  const flow: Flow = { start: "capture", nodes: { capture: { id: "capture", type: "capture", text: "Which model?", variable: "model", next: "gate" }, gate: { id: "gate", type: "condition", condition: { variable: "model", operator: "contains", value: "Rover" }, trueNext: "end", falseNext: "end" }, end: { id: "end", type: "end" } } };
