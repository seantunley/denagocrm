...
const canRun = process.env.NODE_ENV === "test" && dbName.endsWith("_test");

test(
  "lead-to-contact conversion is tenant-safe, access-safe and concurrency-safe",
  { skip: canRun ? false : "requires the CI throwaway *_test database" },
  async () => {
